	...
	async function testApiEndpoints() {
		try {
			...
			const results = await Promise.allSettled(tests.map(test => 
				Promise.race([test, new Promise(resolve => setTimeout(() => resolve('⏱️ Timeout'), 5000))])
			));
			
			apiStatus = results.map(result => 
				result.status === 'fulfilled' ? result.value : '❌ Test fehlgeschlagen'
			).join(' | ');
...
<div class="container mx-auto p-8">
	<div class="space-y-6">
		...
		<div class="card bg-base-100 shadow-xl">
			<div class="card-body">
				<h1 class="card-title text-3xl mb-6">🔍 System Status Dashboard</h1>
