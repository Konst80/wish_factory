<script lang="ts">
	...
	let chartViewMode = $state('days'); // 'days', 'months', 'years'
	...
	// Process wishes over time data based on view mode
	function getProcessedWishesOverTime() {
		if (chartViewMode === 'days') {
			return data.wishesOverTime;
		} else if (chartViewMode === 'months') {
			// Group by month
			const monthlyData: Record<string, number> = {};
			data.wishesOverTime.forEach((item) => {
				...
				if (parts.length >= 2) {
					...
					monthKey = monthNames[month - 1] || item.month;
				}
				monthlyData[monthKey] = (monthlyData[monthKey] || 0) + item.count;
			});
			return Object.entries(monthlyData).map(([month, count]) => ({ month, count }));
		} else if (chartViewMode === 'years') {
			// Group by year (for now, simulate yearly data)
			const currentYear = new Date().getFullYear();
			const yearlyData: Record<string, number> = {};

			// For demonstration, create yearly grouping
			data.wishesOverTime.forEach((item) => {
				const yearKey = currentYear.toString();
				yearlyData[yearKey] = (yearlyData[yearKey] || 0) + item.count;
			});

			// Add previous years with sample data
			for (let i = 1; i <= 2; i++) {
				const year = (currentYear - i).toString();
				yearlyData[year] = Math.floor(Math.random() * 500) + 200; // Sample data
			}
...
<section class="mb-12">
	<div class="divider">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-lg font-semibold">Schnellübersicht</h2>
		</div>
	</div>
...
<section class="mb-12">
	<div class="divider">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-lg font-semibold">Zeitverlauf & Trends</h2>
		</div>
	</div>
		...
		<div class="card bg-base-100 shadow-xl">
			<div class="card-body">
				<div class="flex items-center justify-between">
					...
					<div class="join">
						<button
							class="btn join-item btn-sm {chartViewMode === 'days'
								? 'btn-primary'
								: 'btn-outline'}"
							onclick={() => (chartViewMode = 'days')}
						>
							Tage
						</button>
						<button
							class="btn join-item btn-sm {chartViewMode === 'months'
								? 'btn-primary'
								: 'btn-outline'}"
							onclick={() => (chartViewMode = 'months')}
						>
							Monate
						</button>
						<button
							class="btn join-item btn-sm {chartViewMode === 'years'
								? 'btn-primary'
								: 'btn-outline'}"
							onclick={() => (chartViewMode = 'years')}
						>
							Jahre
						</button>
					</div>
				...
				<div class="h-64 w-full">
					{#if getProcessedWishesOverTime().length > 0}
						{@const processedData = getProcessedWishesOverTime()}
						<!-- Simple bar chart representation -->
						<div class="flex h-full flex-col justify-center gap-1 px-4">
							<!-- Count numbers row -->
							<div class="flex justify-center gap-1">
								{#each processedData as dayData (dayData.month)}
									...
									<div class="flex max-w-12 flex-1 justify-center">
										<div
											...
											style="width: {Math.min(
												processedData.length <= 7 ? 32 : 24,
												48
											)}px; height: {Math.max(
												(dayData.count / maxCount) * 180,
												dayData.count > 0 ? 8 : 2
											)}px; min-height: 2px;"
											title="{chartViewMode === 'days'
												? formatChartLabel(dayData.month).day +
													' ' +
													formatChartLabel(dayData.month).month
												: dayData.month}: {dayData.count} Wünsche"
										></div>
									</div>
								{/each}
							</div>
...
<section class="mb-12">
	<div class="divider">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-lg font-semibold">Content-Verteilung</h2>
		</div>
	</div>
...
<section class="mb-12">
	<div class="divider">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-lg font-semibold">Leistung & Qualität</h2>
		</div>
	</div>
