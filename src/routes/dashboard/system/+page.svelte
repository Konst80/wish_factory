			...
			<div class="space-y-2">
				...
				<div class="text-sm">
					...
					<span class="float-right">
						<div class="badge badge-sm {data.systemInfo.application.environment === 'production' ? 'badge-success' : 'badge-warning'}">
		...
		<div class="card-body">
			...
			<div class="space-y-2">
				...
				<div class="text-sm">
					...
					<span class="float-right">{(data.systemInfo.performance.cpuUsage.user / 1000).toFixed(2)}ms</span>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="card-body">
				<div class="flex items-center justify-between mb-4">
					...
					<div class="badge {getStatusBadgeClass(serviceInfo.status)} gap-2">
						<div class="h-2 w-2 rounded-full {serviceInfo.status === 'online' ? 'bg-success animate-pulse' : 'bg-error'}"></div>
						{serviceInfo.status}
					</div>
				</div>
				<div class="space-y-2">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			{#each Object.entries(data.systemInfo.environment) as [key, value]}
				<div class="flex items-center justify-between p-3 bg-base-200 rounded-lg">
					...
					<span class="text-sm {value && value.includes('[CONFIGURED]') ? 'text-success' : value && value.includes('[NOT CONFIGURED]') ? 'text-error' : ''}">{value || 'N/A'}</span>
				</div>
			{/each}
		</div>
	</div>
	...
	<div class="card-body">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div class="flex items-center justify-between p-3 bg-base-200 rounded-lg">
				...
				<span class="text-sm">{new Date(data.systemInfo.application.buildDate).toLocaleString()}</span>
			</div>
		</div>
	</div>
</div>